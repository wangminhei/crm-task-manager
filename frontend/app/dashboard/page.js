...
// ─── Helpers ─────────────────────────────────────────────────
const STATUS_MAP = {
  todo:        { label: 'Cần làm',    bg: 'bg-orange-100', text: 'text-orange-700', dot: 'bg-orange-400' },
  in_progress: { label: 'Đang làm',   bg: 'bg-yellow-100', text: 'text-yellow-700', dot: 'bg-yellow-400' },
  pending:     { label: 'Chờ duyệt',  bg: 'bg-purple-100', text: 'text-purple-700', dot: 'bg-purple-400' },
  done:        { label: 'Hoàn thành', bg: 'bg-green-100',  text: 'text-green-700',  dot: 'bg-green-400'  },
}
...
function DonutChart({ data }) {
  ...
  const slices = data.map(d => {
    ...
    return {
      ...
      label: STATUS_MAP[d.status]?.label || d.status,
      count: val,
    }
  })
      ...
      <div className="flex bg-gray-100 min-h-screen">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            ...
            <button onClick={fetchStats} className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm">
              Thử lại
            </button>
          </div>
        </div>
    ...
    <div className="flex bg-gray-100 min-h-screen">
      ...
      <div className="flex-1 p-6 overflow-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            onClick={fetchStats}
            className="text-sm text-gray-500 hover:text-gray-800 border border-gray-300 px-3 py-1.5 rounded-lg hover:bg-white transition-colors"
          >
            ↻ Làm mới
          </button>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <StatCard
            ...
            value={fmt(ov.total_tasks)}
            sub={`${fmt(ov.tasks_done)} đã hoàn thành`}
            color="blue"
          />
          <StatCard
            label="Nhân viên"
            value={fmt(ov.total_employees)}
            sub="Đang hoạt động"
            color="purple"
          />
          <StatCard
          ...
          <StatCard
            label="Quá hạn"
            value={fmt(ov.overdue_tasks)}
            sub="Cần xử lý ngay"
            color={fmt(ov.overdue_tasks) > 0 ? 'red' : 'green'}
          />
        </div>
                ...
                return (
                  <div key={p.priority}>
                    <div className="flex items-center justify-between text-sm">
                      <span className={`px-2 py-0.5 rounded text-xs font-medium ${pr.bg} ${pr.text}`}>
                        {pr.label || p.priority}
                      </span>
        ...
        {/* ── ROW 4: TOP EMPLOYEES + TOP CUSTOMERS ── */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1 flex-wrap">
                        ...
                        <span className="text-xs text-red-500">
                          Quá {fmt(t.days_overdue)} ngày
                        </span>
                      </div>
                      {t.employee_name && (
                        <p className="text-xs text-gray-400 mt-1">👤 {t.employee_name}</p>
                      )}
                    </div>
          ...
          <div className="bg-white rounded-xl shadow p-5">
            ...
            {stats.recent_tasks.length === 0 ? (
              <p className="text-gray-400 text-sm">Chưa có task nào</p>
            ) : (
              <div className="space-y-3">
